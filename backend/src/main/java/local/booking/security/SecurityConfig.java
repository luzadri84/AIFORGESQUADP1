...
public class SecurityConfig {
    ...
    @Bean SecurityFilterChain security(HttpSecurity http) throws Exception {
        http.headers(h -> h.withObjectPostProcessor(EagerSecurityHeaders.processor()))
            ...
            .httpBasic(b -> b.authenticationEntryPoint((req,res,e) -> unauthorized(req,res)))
            .exceptionHandling(e -> e.authenticationEntryPoint((req,res,x) -> unauthorized(req,res))
    ...
    private static void unauthorized(jakarta.servlet.http.HttpServletRequest req,jakarta.servlet.http.HttpServletResponse res) throws java.io.IOException {
        // Browser challenge only for protected documentation; API keeps JSON without native login prompts.
        if(req.getRequestURI().startsWith("/swagger-ui")||req.getRequestURI().startsWith("/v3/api-docs"))
            res.setHeader("WWW-Authenticate","Basic realm=\"Booking documentation\"");
        problem(res,401,"Autenticación requerida");
    }
