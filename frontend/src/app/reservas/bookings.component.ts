...
export class BookingsComponent implements OnInit {
 ...
 private fail(error:HttpErrorResponse){if(error.status===409 && Array.isArray(error.error?.rejected)) this.rejected.set(error.error.rejected as BookingResult['rejected']);this.error.set(error.status===409?'El espacio ya está reservado en ese horario.':error.status===400?'Revisa los campos: la fecha final debe ser posterior a la inicial.':error.status===401||error.status===403?'Acceso no válido. Cierra sesión y vuelve a entrar.':'No se pudo confirmar la operación. Actualiza la lista antes de reintentar.');}
}
